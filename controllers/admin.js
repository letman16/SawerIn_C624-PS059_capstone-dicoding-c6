...
const dashboardAdmin = (req, res) => {
    ...
    if (req.session.admin) {
        ...
        let sqlSumTrx = "SELECT SUM(nominal) AS sumTrx, COUNT(*) AS countTrx FROM transaksi_nyawers WHERE status_transaksi = 'Sukses'";
        mysql.conn.query(sqlSumTrx, (err, resultSumTrx) => {
            if (err) {
                console.error(err);
                return res.status(500).send('Error in retrieving transaction sum');
            }
            
            const sumTrx = resultSumTrx[0].sumTrx || 0;
            const countTrx = resultSumTrx[0].countTrx || 0;
            ...
            mysql.conn.query(sqlSumUsers, (err, resultSumUsers) => {
                ...
                let sqlWdPending = "SELECT SUM(nominal) AS sumWDPending, COUNT(*) AS countWDPending FROM dompets WHERE status_transaksi_dompet = 'Pending'";
                mysql.conn.query(sqlWdPending, (err, resultSumWdPending) => {
                    if (err) {
                        console.error(err);
                        return res.status(500).send('Error in retrieving pending withdrawal sum');
                    }
                    
                    const sumWDPending = Math.abs(resultSumWdPending[0].sumWDPending) || 0;
                    const countWDPending = resultSumWdPending[0].countWDPending || 0;

                    res.render("admin/dashboard_admin", { admin: req.session.admin, dataDashboard: { countTrx, sumTrx, sumUsers, countWDPending, sumWDPending } });
                });
            });
        });
    } else {
